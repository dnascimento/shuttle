...
public class NHttpReverseProxy {
    ...
    public static void main(String[] args) throws Exception {
        ...
        HttpProcessor inhttpproc = new ImmutableHttpProcessor(
                new HttpResponseInterceptor[] { new ResponseDate(), new ResponseServer(),
        ...
        HttpAsyncRequester executor = new HttpAsyncRequester(outhttpproc,
                new ProxyOutgoingConnectionReuseStrategy(), params);
        ...
        handlerRegistry.register("*", new ProxyRequestHandler(targetHost, executor,
                connPool));

        ProxyServiceHandler serviceHandler = new ProxyServiceHandler(inhttpproc,
                new ProxyIncomingConnectionReuseStrategy(), handlerRegistry, params);
    ...
    static class ProxyHttpExchange {
        ...
        public ProxyHttpExchange() {
            super();
            inBuffer = ByteBuffer.allocateDirect(10240);
            outBuffer = ByteBuffer.allocateDirect(10240);
        }
        ...
        public void setResponseTrigger(HttpAsyncExchange responseTrigger) {
            responseTrigger = responseTrigger;
        }

        public IOControl getClientIOControl() {
            return clientIOControl;
        }

        public void setClientIOControl(IOControl clientIOControl) {
            clientIOControl = clientIOControl;
        }

        public IOControl getOriginIOControl() {
            return originIOControl;
        }

        public void setOriginIOControl(IOControl originIOControl) {
            originIOControl = originIOControl;
        }
    ...
    static class ProxyRequestHandler
        ...
        public ProxyRequestHandler(HttpHost target,
                HttpAsyncRequester executor,
                BasicNIOConnPool connPool) {
            super();
            target = target;
            executor = executor;
            connPool = connPool;
        ...
        public HttpAsyncRequestConsumer<ProxyHttpExchange> processRequest(
            ...
            synchronized (httpExchange) {
                httpExchange.reset();
                String id = String.format("%08X", counter.getAndIncrement());
                httpExchange.setId(id);
                httpExchange.setTarget(target);
                return new ProxyRequestConsumer(httpExchange, executor, connPool);
            }
        }
    ...
    static class ProxyRequestConsumer
        ...
        public void requestReceived(final HttpRequest request) {
            synchronized (httpExchange) {
                ...
                executor.execute(new ProxyRequestProducer(httpExchange),
                                 new ProxyResponseConsumer(httpExchange),
                                 connPool);
            }
        }
        ...
        public void consumeContent(final ContentDecoder decoder, final IOControl ioctrl) throws IOException {
            synchronized (httpExchange) {
                httpExchange.setClientIOControl(ioctrl);
                // Receive data from the client
                ByteBuffer buf = httpExchange.getInBuffer();
                int n = decoder.read(buf);
                System.out.println("[client->proxy] " + httpExchange.getId() + " " + n
                        + " bytes read");
                ...
                // output is active
                if (buf.position() > 0) {
                    if (httpExchange.getOriginIOControl() != null) {
                        httpExchange.getOriginIOControl().requestOutput();
                        System.out.println("[client->proxy] " + httpExchange.getId()
        ...
        public void requestCompleted(final HttpContext context) {
            synchronized (httpExchange) {
                ...
                System.out.println("[client->proxy] " + httpExchange.getId()
                        + " request completed");
                httpExchange.setRequestReceived();
                if (httpExchange.getOriginIOControl() != null) {
                    httpExchange.getOriginIOControl().requestOutput();
                    System.out.println("[client->proxy] " + httpExchange.getId()
                            + " request origin output");
    ...
    static class ProxyRequestProducer
        ...
        public HttpRequest generateRequest() {
            synchronized (httpExchange) {
                ...
                System.out.println("[proxy->origin] " + httpExchange.getId() + " "
                        + request.getRequestLine());
                // Rewrite request!!!!
        ...
        public void produceContent(final ContentEncoder encoder, final IOControl ioctrl) throws IOException {
            synchronized (httpExchange) {
                httpExchange.setOriginIOControl(ioctrl);
                // Send data to the origin server
                ByteBuffer buf = httpExchange.getInBuffer();
                buf.flip();
                int n = encoder.write(buf);
                buf.compact();
                System.out.println("[proxy->origin] " + httpExchange.getId() + " " + n
                        + " bytes written");
                // If there is space in the buffer and the message has not been
                // transferred, make sure the client is sending more data
                if (buf.hasRemaining() && !httpExchange.isRequestReceived()) {
                    if (httpExchange.getClientIOControl() != null) {
                        httpExchange.getClientIOControl().requestInput();
                        System.out.println("[proxy->origin] " + httpExchange.getId()
                                + " request client input");
                    }
                }
                if (buf.position() == 0) {
                    if (httpExchange.isRequestReceived()) {
                        encoder.complete();
                        System.out.println("[proxy->origin] " + httpExchange.getId()
    ...
    static class ProxyResponseConsumer
        ...
        public void responseReceived(final HttpResponse response) {
            synchronized (httpExchange) {
                ...
                HttpAsyncExchange responseTrigger = httpExchange.getResponseTrigger();
        ...
        public void consumeContent(final ContentDecoder decoder, final IOControl ioctrl) throws IOException {
            synchronized (httpExchange) {
                ...
                ByteBuffer buf = httpExchange.getOutBuffer();
                int n = decoder.read(buf);
                System.out.println("[proxy<-origin] " + httpExchange.getId() + " " + n
                        + " bytes read");
                if (decoder.isCompleted()) {
                ...
                if (buf.position() > 0) {
                    if (httpExchange.getClientIOControl() != null) {
                        httpExchange.getClientIOControl().requestOutput();
                        System.out.println("[proxy<-origin] " + httpExchange.getId()
        ...
        public void responseCompleted(final HttpContext context) {
            synchronized (httpExchange) {
                ...
                System.out.println("[proxy<-origin] " + httpExchange.getId()
                        + " response completed");
                httpExchange.setResponseReceived();
                if (httpExchange.getClientIOControl() != null) {
                    httpExchange.getClientIOControl().requestOutput();
                    System.out.println("[proxy<-origin] " + httpExchange.getId()
                            + " request client output");
    ...
    static class ProxyResponseProducer
        ...
        public void produceContent(final ContentEncoder encoder, final IOControl ioctrl) throws IOException {
            synchronized (httpExchange) {
                ...
                if (buf.hasRemaining() && !httpExchange.isResponseReceived()) {
                    if (httpExchange.getOriginIOControl() != null) {
                        httpExchange.getOriginIOControl().requestInput();
                        System.out.println("[client<-proxy] " + httpExchange.getId()
                                + " request origin input");
                    }
                }
                if (buf.position() == 0) {
                    if (httpExchange.isResponseReceived()) {
                        encoder.complete();
                        System.out.println("[client<-proxy] " + httpExchange.getId()
                                + " content fully written");
                    } else {
    ...
    static class ProxyServiceHandler extends
            HttpAsyncService {
    ...
    static class ProxyClientProtocolHandler extends
        ...
        @Override
        public void connected(final NHttpClientConnection conn, final Object attachment) throws IOException,
                HttpException {
    ...
    static class ProxyConnPool extends
            BasicNIOConnPool {
